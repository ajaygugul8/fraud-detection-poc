...
const HighValueTransactions = ({ data, cypher }) => {
          ...
          <tbody>
            {data.map((item, idx) => (
              <tr key={idx} className="border-b border-gray-700 hover:bg-gray-800 transition-colors">
                ...
                <td className="px-4 py-3 font-mono text-xs">{item.transaction_id}</td>
                <td className="px-4 py-3 text-right font-mono text-yellow-400">
                  ${Number(item.transaction_amount).toFixed(2)}
                </td>
                <td className="px-4 py-3 text-gray-400">{item.transaction_date}</td>
              </tr>
            ))}
          </tbody>
        </table>
